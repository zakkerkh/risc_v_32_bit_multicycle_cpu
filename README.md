...
- **FETCH / FETCH_W**: read the instruction at the PC and set PC = PC + 4. Memory reads are synchronous, so FETCH_W waits a cycle before loading the instruction register.
- **DECODE**: compute the branch target (PC + immediate) in case the instruction turns out to be a branch.
...
3. Compile and run with a simulator such as [Icarus Verilog](https://steveicarus.github.io/iverilog/):
   ```sh
   iverilog -o cpu_sim cpu/*.v your_testbench.v
   vvp cpu_sim
   ```
